...
def analyze_cv(cv_text: str, oferta_text: str, debug: bool = False) -> dict:
    """Full CV-vs-offer analysis. Never raises — returns _DEFAULT_RESULT on error."""
    try:
        return analyze_cv_inner(cv_text, oferta_text, debug=debug)
    except Exception as exc:
        result = dict(_DEFAULT_RESULT)
        result["brechas"] = [f"Error interno durante el análisis: {type(exc).__name__} — {exc}"]
        if debug:
            result["_debug_error"] = traceback.format_exc()
        return result


@functools.lru_cache(maxsize=256)  # cache repeated analyses for identical inputs
def analyze_cv_inner(cv_text: str, oferta_text: str, debug: bool = False) -> dict:
    ...
    # -----------------------------------------------------------------------
    # Composite score
    #   Each category score already represents its max contribution:
    #   tools=35, functional=35, profile=20, clarity=10 → total 100
    # -----------------------------------------------------------------------
    encaje_global = round(min(100.0, max(0.0,
        puntaje_funcional + puntaje_tools + puntaje_perfil + puntaje_claridad
    )), 1)
    ...
    if debug:
        result["_debug_tfidf_raw"] = round(encaje_global_base, 2)
        result["_debug_puntajes_parciales"] = {
            "tfidf_base": round(encaje_global_base, 2),
            "funcional_raw": round(puntaje_funcional, 2),
            "tools_raw": round(puntaje_tools, 2),
            "perfil_raw": round(puntaje_perfil, 2),
            "claridad_raw": round(puntaje_claridad, 2),
        }
        result["_debug_coincidencia_kw"] = round(coincidencia_kw, 3)
        result["_debug_cv_word_count"] = palabras_cv_count
        result["_debug_oferta_word_count"] = len(oferta_clean.split())
        result["_debug_tools"] = {
            "en_oferta": tools_oferta,
            "en_cv": tools_cv,
            "coincidentes": tools_coincidentes,
            "faltantes": tools_faltantes,
        }
        result["_debug_seniority"] = {
            "cv": seniority_cv,
            "oferta": seniority_oferta,
            "coincide": bool(seniority_cv and seniority_oferta and seniority_cv == seniority_oferta),
        }
        result["_debug_logros"] = num_logros
        result["_debug_tiene_fechas"] = tiene_fechas
...
def simulate_score(cv_text: str, oferta_text: str, additions: list[str]) -> dict:
    """Returns current score vs projected score after adding hypothetical keywords."""
    modified_cv = cv_text + "\n" + "\n".join(additions)
    current = analyze_cv_inner(cv_text, oferta_text)
    projected = analyze_cv_inner(modified_cv, oferta_text)
    return {
        "actual": current["encaje_global"],
        "proyectado": projected["encaje_global"],
        "diferencia": round(projected["encaje_global"] - current["encaje_global"], 1),
    }
...
def _generar_frases_ejemplo(tools_faltantes: list, func: dict) -> list:
    """Generate up to 3 example CV bullet points addressing the top gaps."""
    frases = []

    if tools_faltantes:
        tool = h_display(tools_faltantes[0])
        frases.append(
            f"Automaticé el proceso de generación de reportes usando {tool}, "
            f"reduciendo el tiempo de procesamiento en un 40% y mejorando la precisión de los datos."
        )
        if len(tools_faltantes) > 1:
            tool2 = h_display(tools_faltantes[1])
            frases.append(
                f"Implementé un pipeline de extracción y análisis con {tool} y {tool2}, "
                f"logrando integrar datos de 3 fuentes distintas en un solo tablero de control."
            )

    if func.get("verbos_faltantes"):
        verbo = func["verbos_faltantes"][0]
        frases.append(
            f"{verbo.capitalize()} los datos de ventas de la región durante 2 años, "
            f"identificando patrones de estacionalidad que permitieron ajustar la estrategia comercial."
        )
        if len(func["verbos_faltantes"]) > 1:
            verbo2 = func["verbos_faltantes"][1]
            frases.append(
                f"Colaboré con el equipo de finanzas para {verbo2} los reportes mensuales, "
                f"reduciendo errores de consolidación en un 25%."
            )

    if func.get("sustantivos_faltantes"):
        sust = func["sustantivos_faltantes"][0].replace('_', ' ')
        frases.append(
            f"Diseñé e implementé un sistema de {sust} que permitió reducir los tiempos "
            f"de cierre mensual en un 30%."
        )

    # Always return 3 phrases — pad with fallbacks if fewer were generated
    FALLBACKS = [
        "Automaticé procesos clave del área usando herramientas de análisis de datos, logrando una reducción del 35% en tiempos de entrega.",
        "Implementé un tablero de control con KPIs estratégicos consultado semanalmente por la alta dirección.",
        "Lideré la migración de reportes manuales a un sistema automatizado, eliminando 15 horas de trabajo repetitivo al mes.",
    ]
    while len(frases) < 3:
        frases.append(FALLBACKS[len(frases) % len(FALLBACKS)])

    return frases[:3]


def generar_sugerencias(cv_text: str, oferta_text: str) -> dict:
    """Generate modification suggestions based on gaps found between CV and offer."""
    cv_text = (cv_text or "").strip()
    oferta_text = (oferta_text or "").strip()
    ...
    herramientas_faltantes = [
        {
            "herramienta":   h_display(h),
            "recomendacion": f"Incluye '{h_display(h)}' en tu sección de habilidades técnicas",
        }
        for h in tools_faltantes
    ]
    ...
    verbos_faltantes = [
        {
            "verbo":         v,
            "recomendacion": f"Incluye el verbo de acción '{v}' en tus logros profesionales",
        }
        for v in func["verbos_faltantes"]
    ]
    sustantivos_faltantes = [
        s.replace('_', ' ').title() for s in func["sustantivos_faltantes"]
    ]

    frases_ejemplo = _generar_frases_ejemplo(tools_faltantes, func)

    return {
        "herramientas_faltantes": herramientas_faltantes,
        "verbos_faltantes":       verbos_faltantes,
        "sustantivos_faltantes":  sustantivos_faltantes,
        "frases_ejemplo":         frases_ejemplo,
    }
...
def generar_preguntas_entrevista(cv_text: str, oferta_text: str) -> list:
    """Generate interview questions — exactly 5 categories × 2 questions each = 10 total."""
    cv_text = (cv_text or "").strip()
    oferta_text = (oferta_text or "").strip()
    ...
    titulo_clave, titulo_display = _detectar_titulo_vacante(oferta_clean)
    seniority_oferta = next((s for s in ['junior', 'semi senior', 'semisenior', 'senior', 'lead'] if s in oferta_clean), None)
    funcion = _detectar_funcion_principal(oferta_clean)
    func = _analizar_palabras_funcionales(cv_clean, oferta_clean)

    verbs = func["verbos_faltantes"]
    nouns = func["sustantivos_faltantes"]

    tools_oferta = herramientas_en_oferta(oferta_clean)
    tools_cv = [h for h in (HERRAMIENTAS_BASE + (['sas'] if 'sas' in oferta_clean else [])) if h in cv_clean]
    tools_faltantes = [h for h in tools_oferta if h not in tools_cv]

    # ----------------------------------------------------------------
    # Category 1: HERRAMIENTA (2 questions)
    def _tool_q(verb, h):
        d = h_display(h)
        return f"¿{verb} con {d}? ¿En qué contexto laboral lo has utilizado y qué resultados obtuviste?"
    candidates_t = []
    not_found = tools_faltantes[:]
    found = tools_oferta[:]
    candidates_t.append(_tool_q("Qué experiencia tienes", not_found[0])) if len(not_found) > 0 else None
    candidates_t.append(_tool_q("Cuál es tu nivel de expertise en", found[0])) if len(found) > 0 else None
    candidates_t.append(_tool_q("Has trabajado", found[1] if len(found) > 1 else found[0]))
    for q in candidates_t:
        preguntas.append({"categoria": "herramienta", "pregunta": q})

    # ----------------------------------------------------------------
    # Category 2: VERBO_ACCION (2 questions)
    def _verb_q(v):
        return f"Cuéntame sobre una ocasión en la que hayas tenido que {v} datos o procesos. ¿Qué herramientas usaste y qué resultado obtuviste?"
    for v in verbs[:2]:
        preguntas.append({"categoria": "verbo_accion", "pregunta": _verb_q(v)})
    while len([p for p in preguntas if p["categoria"] == "verbo_accion"]) < 2:
        fallbacks = ["optimizar", "automatizar", "implementar", "analizar", "diseñar"]
        for fb in fallbacks:
            if len([p for p in preguntas if p["categoria"] == "verbo_accion"]) < 2:
                preguntas.append({"categoria": "verbo_accion", "pregunta": _verb_q(fb)})

    # ----------------------------------------------------------------
    # Category 3: CONCEPTO (2 questions)
    def _noun_q(n):
        display = n.replace('_', ' ').title()
        return f"¿Has trabajado con {display}? ¿Cómo lo has aplicado en proyectos previos y qué impacto tuvo?"
    for s in nouns[:2]:
        preguntas.append({"categoria": "concepto", "pregunta": _noun_q(s)})
    while len([p for p in preguntas if p["categoria"] == "concepto"]) < 2:
        temas = ["machine learning", "cloud computing", "ciencia de datos", "metodologías ágiles", "microservicios"]
        for t in temas:
            if len([p for p in preguntas if p["categoria"] == "concepto"]) < 2:
                preguntas.append({"categoria": "concepto", "pregunta": _noun_q(t)})

    # ----------------------------------------------------------------
    # Category 4: ROL (2 questions)
    rol_qs = [
        f"¿Por qué crees que eres un buen candidato para el rol de {titulo_display}? ¿Qué te diferencia de otros postulantes?",
        f"Si fueras contratado como {titulo_display}, ¿cuál sería tu plan de acción para los primeros 90 días?",
    ] if titulo_display else [
        "¿Qué tipo de rol buscas en tu próxima posición y por qué?",
        "Describe cómo has colaborado con equipos multidisciplinarios en tu experiencia reciente.",
    ]
    for q in rol_qs[:2]:
        preguntas.append({"categoria": "rol", "pregunta": q})

    # ----------------------------------------------------------------
    # Category 5: SENIORITY (2 questions)
    senior_qs = [
        f"La vacante busca un perfil {seniority_oferta}. ¿Cómo describirías tu nivel de experiencia y autonomía en tu área?",
        f"En un rol {seniority_oferta}, ¿qué tipo de responsabilidades esperas asumir y cómo te preparaste para ello?",
    ] if seniority_oferta else [
        "¿Cómo describirías el nivel de autonomía con el que trabajas actualmente?",
        "¿Has tenido experiencia liderando proyectos o mentorizando a otros? Cuéntame sobre eso.",
    ]
    for q in senior_qs[:2]:
        preguntas.append({"categoria": "seniority", "pregunta": q})
